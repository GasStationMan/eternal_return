...
import org.EternalReturn.Util.DPEngine.Geometry.GeometryCalculatable;
import org.EternalReturn.Util.DPEngine.Geometry.MatVecCalculator;
...
public abstract class Monobehaviour<T extends MonobehaviourEvent> extends GeometryCalculatable {
    ...
    public void updateMonobehav(Collection<MonobehaviourEvent> eventList){
        try(MatVecCalculator.VecScope scope = actor.engine.setVecScope()){
            update(eventList);
        }//여기서 update()에 사용되었던 임시 벡터들이 모두 free됨.
    }
    ...
    public abstract void update(Collection<MonobehaviourEvent> eventList);

    /**
     * MonobehaviourActor 내 스케줄러로 하여금 해당 Monobehaviour을 제거하도록 마킹합니다. <br>
     * 스케줄러에서 제거된 Monobehaviour은 update() 대상에서 제외됩니다. <br>
    ...
    protected MonobehaviourActor getActor(){
        return this.actor;
    }

    /**
     * 해당 Monobehaviour을 실행할 Actor을 적용함과 동시에 <br>
     * 해당 Monobehaviour에서 사용할 GeometryEngine을 적용하는 함수 <br>
