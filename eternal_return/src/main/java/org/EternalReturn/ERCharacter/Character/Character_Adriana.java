...
public class Character_Adriana extends ERCharacter {
    ...
    private LinkedList<BurningGround> burningGroundList = new LinkedList<>();
    @Override public void update() {

        LinkedList<BurningGround> newBGList = new LinkedList<>();

        while(!burningGroundList.isEmpty()){
            BurningGround burningGround = burningGroundList.removeFirst();
            burningGround.update(); //업데이트
            if(isNotEnd(burningGround.startTime, 60)){
                newBGList.add(burningGround); //아직 시간이 남았다면 넣기
            }
            //시간이 다 끝났다면 버리기
        }
        burningGroundList = newBGList; //갱신
    ...
    @Override
    public void onActiveSkill(ERPlayer erPlayer) {
    ...
    class BurningGround{
        ...
        public BurningGround(Player caster, Location location, long startTime){
    ...
    // [3] 특정 지점에 3초 동안 유지되는 불길 생성 메서드
    private void createBurningGround(Player caster, Location loc) {
        burningGroundList.add(new BurningGround(caster, loc, System.currentTimeMillis()));
    }


    // [4] 특정 지점에 3초 동안 유지되는 불길 업데이트 메소드
