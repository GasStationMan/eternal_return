...
abstract class EREntity( // extends MonobehaviourActor()
     ...
     */
    val collider: Collider) : MonobehaviourActor() {

    val motionManager : MotionManager = MotionManager(entity)

    val geometryModule : GeometryModule = this.monobehaviourModule.dpEngine.geometryModule;

    init {
        /**이거 존나 위험한 구문임. 나중에 어떻게든 수정해야 할 것. */
        PluginInstance.getEREngine().monobehaviourModule.registerMonobehaviourActor(this)
        /**이거 존나 위험한 구문임. 나중에 어떻게든 수정해야 할 것. */
