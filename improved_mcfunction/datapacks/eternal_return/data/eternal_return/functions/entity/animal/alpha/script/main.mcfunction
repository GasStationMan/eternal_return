...
scoreboard players operation #this.HP ER.sys = @n[type=minecraft:ghast       ,tag=ER.animal.hitbox,tag=this] ER.health
...
execute if score #this.HP ER.sys matches 1.. run return run function eternal_return:entity/animal/alpha/script/b2/code
execute if score #this.HP ER.sys matches ..0 run function eternal_return:entity/animal/alpha/script/b3/code
data modify entity @n[type=minecraft:ghast       ,tag=ER.animal.hitbox,tag=this] Pos set from entity @s Pos
