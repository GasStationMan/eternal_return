...
execute at @s as ModelEntity with Animal_info :
    ...
    $if entity @s[tag= aj.animal_$(animal).animation.ready] run\
        function animal:ready_stop


    #ATTACK 애니메이션 실행 및 로직 발동 Cooltime >= 100 까지
    if Cooltime == 61.. run goto with Animal_info:
        ...
        $if entity @p[tag=targeted, distance=..$(attackDistance)] with Animal_info :
            if entity @s[tag= aj.animal_$(animal).animation.attack,\
                         scores={aj.anim_time = $(attackTick)}] run return run\
        ...
        #MOVE 애니메이션 종료
        $if isMotionExist == 0\
            if entity @s[tag= aj.animal_$(animal).animation.move] run\
                function animal:move_stop

        #MOVE 애니메이션 실행 (스킬 사용 중 제외)
        $if isMotionExist == 1\
            if entity @s[tag=!aj.animal_$(animal).animation.move,\
                          tag=!aj.animal_$(animal).animation.attack,\
                          tag=!aj.animal_$(animal).animation.skill] run\
                function animal:move_play

    #0틱 때 스킬 발동
    if Cooltime == 60 run goto with Animal_info:
        say skill started
