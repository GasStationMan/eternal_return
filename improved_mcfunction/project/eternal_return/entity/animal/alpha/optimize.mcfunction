...
execute at @s[tag= ER.optimized] if entity @p[distance=..20] run goto:
    ...
    execute as @e[tag=aj.animal_alpha.root] if score @s aj.id = aj.last_id aj.id:
        tag @s += this
        tag @s += ER
        tag @s += ER.animal
        tag @s += ER.animal.model
        tag @s += ER.animal.alpha
...
execute at @s[tag=!ER.optimized] unless entity @p[distance=..21]:
    tag @s += ER.optimized
    execute as ModelEntity run function animated_java:animal_alpha/remove/this
