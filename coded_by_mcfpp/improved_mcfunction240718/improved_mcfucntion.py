...
def generate_mcfunctions(depth,code,mcf_output_dir,FILE_NAME):
    ...
    LastFileName[0] = mcf_output_dir[mcf_output_dir.index("functions\\") + len("functions\\"):] + "\\" + LastFileName[0]
        ...
        if i > 0 :
            ...
            if before__code.node == 1 :
                if before__code.depth < current_code.depth : #execute 또는 function : 콜론 이후 진입 후
                    ...
                    OutputFunctionArg = " ".join(before__code.FUN_Arg)
                    if OutputFunctionArg == "" : 
                        OutputFunctionDir = f'{bs0.join(LastFileName[0].split(bs1))}{bs0}{bs0.join(List_Dir)}{bs0}code{LB}'
                    else                       : 
                        OutputFunctionDir = f'{bs0.join(LastFileName[0].split(bs1))}{bs0}{bs0.join(List_Dir)}{bs0}code {OutputFunctionArg}{LB}'
                    #정해진 파일에 쓰기
                ...
                if before__code.depth > current_code.depth : #depth를 통해 execute 또는 function 탈출 감지 
                    for j in range(before__code.depth - current_code.depth): List_Dir.pop()
                if current_code.node == 2 : 
                    OutputFunctionDir = f"{bs0.join(LastFileName[0].split(bs1))}{bs0}{bs0.join(List_Dir)}{bs0}code {OutputFunctionArg}"
