...
Animal_Information = {animal : "alpha", OPTdistance : 20, damage : 5, attackTick : 12, attackDistance : 5}
...
function with Animal_Information :
    $execute as @e[tag= ER.animal.$(animal)] if Find_With_This_ID = @s df_id :
        if entity @s[type= minecraft:ghast,       tag=ER.animal.hitbox] run return run tag @s += this
        if entity @s[type= minecraft:item_display,tag=ER.animal.model ] run return run tag @s += this
        if entity @s[type= minecraft:text_display,tag=ER.animal.HPbar ] run return run tag @s += this
        if entity @s[type= minecraft:zombie,      tag=ER.animal.root  ] run return run tag @s += this
