...
class code_line:
            ...
            if state == 0 :
                ...
                if   VARTYPE == "storage" :
                    ...
                    if self.tokens[i+1] == "score" : 
                        if self.tokens[i+3] == "=":
                            ...
                            if VARINFO_2[var_type] == "score" and MAX_LENGTH == 5:
                                ...
                                if MAX_LENGTH == 5 :
                                    ...
                                    return True
                                elif MAX_LENGTH == 6 and isDigit(self.tokens[i+5]) :
                                    self.replaceVAR([VARINFO_2],i+4)
                            ...
                            elif VARINFO_2[var_type] == "entity" and MAX_LENGTH >= 7:
                                if self.tokens[i+5] == "score" :
                                    ...
                                    if MAX_LENGTH == 7:
                                        self.data = "execute store result score %s %s run data get entity %s %s"%\
                                                (VARINFO[entity],self.tokens[i+2],VARINFO_2[entity],self.tokens[i+6])
                                    elif MAX_LENGTH == 8 and isDigit(self.tokes[i+7]):
                                        self.data = "execute store result score %s %s run data get entity %s %s %s"%\
                                                (VARINFO[entity],self.tokens[i+2],VARINFO_2[entity],self.tokens[i+6])
                                    else : break
                                else : break
                            elif isDigit(self.tokens[i+4]) and MAX_LENGTH == 5 :
                        ...
                        if self.tokens[i+3] == "=":
                            VARINFO_2 = self.isVAR(VAR_TABLE,i+4)
                            if VARINFO_2[0] == "score" and MAX_LENGTH >= 5:
                                if MAX_LENGTH == 5 :
                                    self.data = "execute store result entity %s %s int 1 run scoreboard players get %s %s"%\
                                                (VARINFO[entity],self.tokens[i+2],VARINFO_2[player],VARINFO_2[scoreboard])
                                    return True
                                elif MAX_LENGTH == 6 and isType(self.tokens[i+5]):
                                    ...
                                    return True
                                elif MAX_LENGTH == 7 and isType(self.tokens[i+5]) and isDigit(self.tokens[i+6]):
                            ...
                            elif VARINFO_2[0] == "storage" and MAX_LENGTH == 5:
                                self.replaceVAR([VARINFO_2],i+4)
                                self.data = "data modify entity %s %s set from %s"%\
                                            (VARINFO[entity],self.tokens[i+2],self.tokens[i+4])
                                return True
                            
                            elif VARINFO_2[0] == "entity" and MAX_LENGTH == 7:
                ...
                #                 i             i = 8
                elif VARTYPE == "score" and MAX_LENGTH >= 3:
                    if self.tokens[i+1] == "=":
                        ...
                        elif VARINFO_2[0] == "entity" and MAX_LENGTH >= 5 and self.tokens[i+3] == "nbt":
                            if MAX_LENGTH == 5 :
                                self.data = "execute store result score %s %s run data get entity %s %s"%\
                        ...
                        elif VARINFO_2[0] == "entity" and MAX_LENGTH == 5 and self.tokens[i+3] == "score":
                            self.data = "scoreboard players operation %s %s %s %s %s"%\
            ...
            ## execute syntax
                ...
                if   token == "as"      : state = 31
                    ...
                    if   token == "attacker"   : state = 3
                                          ...
                                          temp_tokens = []
                                          temp_tokens = self.tokens[:i+1]
                                          temp_tokens[i] = "run function"
                                          temp_tokens.append("with")
                                          temp_tokens = temp_tokens + self.tokens[i+1:]
                    ...
                    elif isDigit(self.tokens[i+1]):
                        if isDigit(self.tokens[i+2]):
                            if isDigit(self.tokens[i+3]):
                                i += 3
                                state = 3
                            else : break
                        else : break
                ...
                if self.isVAR(VAR_TABLE,i)[0] == "entity" :
                                               self.replaceVAR(VAR_TABLE,i)
                                               state = 3
                elif isTargetSelector(token) : state = 3
                else : break
                ...
                elif token == "block"   : state = 31 #일단 우회
    ...
    def callParser(self,VAR_TABLE,row):
        #문법 감지
        if self.syntaxCheck(VAR_TABLE,row) == True : 
