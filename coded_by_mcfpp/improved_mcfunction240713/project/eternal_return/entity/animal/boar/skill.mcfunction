...
execute on passengers at @s :
    ...
    execute as @e[type=minecraft:marker,tag=getTarget] at @s :
        tp @s ^ ^ ^1
        if entity @a[tag=! skillTargeted, distance=..2] run tag @a[tag=! skillTargeted, distance=..2] += skillTargeted
        i += 1
        if i matches ..9 as @s at @s run goto BACK
...
execute as @a[tag=skillTargeted] at @s summon minecraft:snowball :
    ...
    tag @a[tag=skillTargeted] -= skillTargeted
