...
execute if score #j2 sys < #len sys run function test:test/test/test/0 with storage minecraft:temp temp
scoreboard players add #i sys 1
execute store result storage minecraft:temp temp.i int 1 run scoreboard players get #i sys
execute if score #i sys < #len sys run return run function test:test/test/0 with storage minecraft:temp temp
